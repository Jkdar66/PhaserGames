...
export class Enemy extends Phaser.GameObjects.Graphics{
    ...
    colliedEnemies(enemies: Enemy[]): boolean{
        var selfPos = {
            x1: this.x, 
            y1: this.y,
        ...
        for (let i = 0; i < this.player.bullets.length; i++) {
            ...
            if(bltPos.y1 < enemyPos.y2) {
                if(bltPos.x2 >= enemyPos.x1 && bltPos.x1 <= enemyPos.x2) {
                    ...
                    if(Math.floor(this.alpha*10)/10 <= 0) { this.isKilled = true; }
                    i--;
                }
            }
        }
        ...
        for (let i = 0; i < this.bullets.length; i++) {
            ...
            if(bltPos.y2 > playerPos.y1) {
                if(bltPos.x2 >= playerPos.x1 && bltPos.x1 <= playerPos.x2) {
                    ...
                    bullet.destroy(true);
                    this.bullets.splice(i, 1);
                    if(Math.floor(this.player.alpha*10)/10 <= 0) { this.player.isKilled = true; }
                    i--;
                }
            }
        }
...
export class EnemyBullet extends Phaser.GameObjects.Graphics{
    ...
    height: number;
    radius: number;
    constructor(enmey: Enemy, color: number) {
